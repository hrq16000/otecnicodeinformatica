-- 1) Contrato de QA no banco (espelha src/lib/qaExclusion.ts)
...
AS $$ SELECT '2026-08-08T00:10:00Z'::timestamptz $$;

CREATE OR REPLACE FUNCTION public.is_qa_click_event(
  _created_at timestamptz, _utm_source text, _utm_medium text,
  _utm_campaign text, _session_id text
...
-- 2) Consolidação comercial: exclui QA/pré-baseline ANTES de agrupar/generalizar/k=5
...
-- 3) purge_raw: cobertura de consolidação por run auditada (evita deadlock em dias 100% QA)
...
-- 4) Correção mínima e auditável do primeiro agregado (07/08 = 100% pré-baseline/QA)
WITH del AS (
  DELETE FROM public.click_events_daily
  WHERE event_date < (public.telemetry_baseline_comercial() AT TIME ZONE 'UTC')::date
  RETURNING 1
)
INSERT INTO public.telemetry_retention_runs (run_type, dry_run, period_end, rows_deleted, outcome, details)
SELECT 'consolidate', false, (public.telemetry_baseline_comercial() AT TIME ZONE 'UTC')::date - 1,
       count(*)::int, 'ok',
       jsonb_build_object('reason', 'remocao de agregado pre-baseline/QA (rodada 4E.5.1)',
                          'raw_preservado', true)
FROM del;

-- 5) Autoteste fail-closed em transação descartável (nada é apagado de verdade)
...
DECLARE
  ...
  v_com integer; v_qa integer; v_pre integer;
  d integer; b integer; c integer;
BEGIN
  BEGIN
    -- Fixture QA x COMERCIAL (mesmo dia/rota, pós-baseline)
    INSERT INTO public.click_events (created_at, event_type, path, session_id, utm_source)
    SELECT now(), 'selftest_click', '/__selftest__', 'sess-com-' || g, NULL FROM generate_series(1,5) g;
    INSERT INTO public.click_events (created_at, event_type, path, session_id, utm_source)
    SELECT now(), 'selftest_click', '/__selftest__', 'sess-qa-' || g, 'qa' FROM generate_series(1,5) g;
    -- Fixture pré-baseline
    INSERT INTO public.click_events (created_at, event_type, path, session_id)
    SELECT public.telemetry_baseline_comercial() - interval '1 day', 'selftest_pre', '/__selftest_pre__', 'sess-pre-' || g
    FROM generate_series(1,5) g;

    PERFORM public.consolidate_click_events(current_date);

    SELECT coalesce(sum(event_count),0) INTO v_com FROM public.click_events_daily
      WHERE event_type = 'selftest_click';
    SELECT coalesce(sum(event_count),0) INTO v_pre FROM public.click_events_daily
      WHERE event_type = 'selftest_pre';
    v_qa := 0;

    v_out := v_out || jsonb_build_object(
      'fixture_comercial_count', v_com,
      'fixture_qa_incluido', (v_com > 5),
      'fixture_pre_baseline_count', v_pre);

    -- Idempotência
    PERFORM public.consolidate_click_events(current_date);
    SELECT coalesce(sum(event_count),0) INTO v_qa FROM public.click_events_daily
      WHERE event_type = 'selftest_click';
    v_out := v_out || jsonb_build_object('idempotencia_run2_count', v_qa);

    -- Fail-closed A: raw elegível + dia sem consolidação
    INSERT INTO public.click_events (created_at, event_type, path, session_id)
    SELECT v_old + time '10:00', 'selftest_old', '/__selftest_old__', 'sess-old-' || g
    ...
    -- Fail-closed B: consolidação válida + nenhum dry-run prévio
    ...
    -- Caminho permitido (fixture descartável)
...
REVOKE ALL ON FUNCTION public.telemetry_guard_selftest() FROM PUBLIC;
